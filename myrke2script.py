...
# Define your domain
domain = "example.com"
myparamiko.generate_hosts_file(routers,domain)
def backup(router):
    try:
        ...
        myparamiko.scpfile(client,'hosts','/tmp/hosts')
        # Additional commands and operations
        myparamiko.send_command(shell,'mv -f /tmp/hosts /etc/hosts')
        ...
        file_name = f'{router_copy["server_ip"]}_{year}-{mounth}-{day}.txt'
        print(file_name)
